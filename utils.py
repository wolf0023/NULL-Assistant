...
# threadが存在するか確認
def is_thread_exist(thread_id: int) -> bool:
...
# threadの履歴の書き込み
def store_thread_history(data: dict, thread_id: int) -> None:
    log.debug("スレッドを保存中...")
...
# threadの履歴の読み込み
def load_thread_history(thread_id: int) -> dict:
    log.debug("スレッドを読み込み中...")
    # 履歴が存在しない場合はNoneを返す
    if not is_thread_exist(thread_id):
        return None
    # jsonファイルを読み込む
    file_path = os.path.join("data", f"{thread_id}.json")
    try:
...
# 保存形式に変換
def convert_to_data(role: str, text: str) -> dict:
...
def remove_unnecessary_line(text: str):
    text = text.replace("``` ", "```")
    text = text.replace("```\n\n", "```\n")
    text = text.replace("\n\n```", "\n```")
    text = text.replace("\n\n", "\n")
...
# DuckDuckGo検索
def search_on_ddgs(word: str, max: int):
        try:
...
# 送信者の名前を挿入
def insert_user_name(user_name: str, user_input: str) -> str:
